...
public enum Direction {
	NORTH {
		...
		int getFrontValue( Maze model ) {
			if( model.getY() - 1 < 0 ) return Maze.HOLE;
			return model.getCellState( model.getX(), model.getY() - 1 );
		}
		...
		boolean isFrontClear( Maze model ) {
			return getFrontValue( model ) < Maze.HOLE;
		}
	...
	EAST {
		...
		@Override
		int getFrontValue( Maze model ) {
			if( model.getX() + 1 > model.getWidth() - 1 ) return Maze.HOLE;
			return model.getCellState( model.getX() + 1, model.getY() );
		}
	...
	SOUTH {
		...
		@Override
		int getFrontValue( Maze model ) {
			if( model.getY() + 1 > model.getHeight() - 1 ) return Maze.HOLE;
			return model.getCellState( model.getX(), model.getY() + 1 );
		}
	...
	WEST {
		...
		@Override
		int getFrontValue( Maze model ) {
			if( model.getX() - 1 < 0 ) return Maze.HOLE;
			return model.getCellState( model.getX() - 1, model.getY() );
		}
